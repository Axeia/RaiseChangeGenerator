...
public class Generator : IIncrementalGenerator
{
    ...
    static string GenerateProxyProperties(List<ProxyPropertyInfo> infos)
    {
        ...
        if (!string.IsNullOrEmpty(first.Namespace)) sb.AppendLine($"namespace {first.Namespace} {{");

        var classParts = first.ContainingClass.Split('.');
        foreach (var part in classParts) sb.AppendLine($"    partial class {part} {{");
        ...
        foreach (var info in infos)
        {
            ...
            foreach (var also in info.AlsoNotifyProperties) sb.AppendLine($"                    this.RaisePropertyChanged(nameof({also}));");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
        }
        ...
        for (int i = 0; i < classParts.Length; i++) sb.AppendLine("    }");
        if (!string.IsNullOrEmpty(first.Namespace)) sb.AppendLine("}");
        return sb.ToString();
    }
