...
public class Generator : IIncrementalGenerator
{
    ...
    static string GenerateReactivePropertiesForClass(List<FieldInfo> fields)
    {
        ...
        {
            // If your project uses file-scoped namespaces, this is fine, 
            // but let's stick to the block-scoped for generated files as it's safer.
    ...
    static bool IsFieldWithRaiseChange(SyntaxNode node) => node is FieldDeclarationSyntax f 
        && f.AttributeLists.SelectMany(al => al.Attributes).Any(a => a.Name.ToString().Contains("RaiseChange"));
    static bool IsFieldWithRaiseChangeProxy(SyntaxNode node) => node is FieldDeclarationSyntax f 
        && f.AttributeLists.SelectMany(al => al.Attributes).Any(a => a.Name.ToString().Contains("RaiseChangeProxy"));
    ...
    static List<ProxyPropertyInfo> GetProxyPropertyInfos(GeneratorSyntaxContext context)
    {
        ...
        {
            var path = attr.ConstructorArguments.Length >= 1 
                ? attr.ConstructorArguments[0].Value?.ToString() ?? "" 
                : "";
            var custom = attr.ConstructorArguments.Length >= 2 
    ...
    static ISymbol? FindPropertyOrFieldInTypeHierarchy(ITypeSymbol type, string memberName)
    {
        ...
        return type.AllInterfaces
            .SelectMany(i => i.GetMembers(memberName))
    ...
    static bool ClassHasReactiveUISupport(INamedTypeSymbol symbol)
    {
        ...
        while (current != null)
        {
            ...
            || current.AllInterfaces.Any(i => i.Name == "IReactiveNotifyPropertyChanged" 
    ...
    static string ToPascal(string name) => name.TrimStart('_') is string t && t.Length > 0 
        ? char.ToUpper(t[0]) + t.Substring(1) 
        : name;
}